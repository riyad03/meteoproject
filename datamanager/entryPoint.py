...
async def create_process_dict(data: Dict[str, Any]):
    startImport(data)'''

class ProcessDict(BaseModel):
    name: str
    description: str
    import_type: str
    fields: str
...
'''
@app.post("/process/dict/")
async def create_process_dict(request: Request,file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"error": "Invalid JSON", "raw_body": body.decode('utf-8') if body else "Empty body"}
    
    result = startImport(data)
