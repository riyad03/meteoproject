...
if __name__ == "__main__":
    ...
    TARGET_API_URL = "http://localhost:5000/api/3/action/package_create" # Example public API for testing POST
    ...
    CUSTOM_HEADERS = {
        "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJFWGtSU2lmSlh5blNTR0pCVnVxYnRvNTJPOFZpUjNCT0lTTFVxbUhJNUJvIiwiaWF0IjoxNzUzNjUyODM1fQ.CJrkpl1oAOXCfys7EMMak6fXYi_KllA2CfGxu_ElJNA"#os.getenv("MY_API_KEY", "YOUR_API_KEY_HERE")
    }

    # 3. Define the data payload you want to send (as a Python dictionary)
    # This dictionary will be converted to JSON automatically.
    DATA_TO_PUSH = {
        "name": "my-new-dataset-from-template2", # Must be unique, lowercase, no spaces
        "title": "My New Dataset Created via Generic API Script",
        "notes": "This dataset was pushed using the general API service template.",
        "author": "Riyad",
        "license_id": "cc-by",
        "tags": [{"name": "template"}, {"name": "test"}],
        "owner_org": "dgm", # Set to an organization ID if applicable, e.g., "my-org-id"
        "resources": [
            {
                "name": "sample-data-file.csv",
                "url": "http://example.com/sample_data.hdf5",
                "format": "CSV",
